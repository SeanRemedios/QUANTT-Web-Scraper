...
class Browser():
	...
	# Login to quantopian
	def login(self):
		url = 'https://www.quantopian.com/signin'
		self.go(url)

		username = self.browser.find_element_by_id("user_email") # Username form field
		password = self.browser.find_element_by_id("user_password") # Password form field

		username.send_keys(self.payload[EMAIL_KEY])
		password.send_keys(self.payload[PASSWORD_KEY])

		submitButton = self.browser.find_element_by_id("login-button") 
		submitButton.click()
